...
def retrain_model(netF_list, netB_list, netC_list, netD, optim_group, optim_domain, pseudo_label):
    ...
    class TempSet(Dataset):
        ...
        def __getitem__(self, index):
            ...
            return data, label, index  # 为了和cifar100c的代码对齐，他是data,label,path

        def __len__(self):
            return len(self.data_list)
    
    mask = pseudo_label >= 0
    temp_loader = load_imagenetc(1, 5, args.t_dset_path, False, [args.name_tar], prepr='train', batch_idx=args.batch_idx)
    l_ls, u_ls = [], []
    for i, (data, label, _) in enumerate(temp_loader):
        if mask[i]:
...
def train_target(args):
    ...
    print(pseudo_label)
    print(pseudo_label.size(), pseudo_label_raw.size())
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dset', type=str, default='office', choices=['office', 'office-home', 'office-caltech', 'imagenetc'])
    ...
    for t in range(3, 5):
        ...
        for i in range(5000//50):
            ...
            f = open(f'ImageNetC_ggsj_target-{args.name_tar}_bs{args.batch_size}_iter{args.max_iterations}.txt', 'a')
            f.write(f'{str(acc)}\n')
            f.close()
            t2 = time.time()
